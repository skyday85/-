...
class OAuthProviderClient:
    ...
    def list_accounts(self, user_id: str) -> list[dict[str, Any]]:
        return [{"account_id": x.account_id, "address": x.address, "display_name": x.display_name} for x in self.store.list_provider(user_id, self.provider)]

    def connection_state(self, user_id: str, account_id: str) -> dict[str, Any]:
        item = self.store.get(user_id, self.provider, account_id)
        return {"connected": True, "scopes": list(item.scopes), "reauth_required": not bool(item.refresh_token)}

    def authorization_url(self, *, redirect_uri: str, state: str, scopes: tuple[str, ...]) -> str:
        raise NotImplementedError

    def complete_authorization(self, *, user_id: str, code: str, redirect_uri: str, scopes: tuple[str, ...]) -> dict[str, Any]:
        raise NotImplementedError

    def fetch_messages(self, user_id: str, account_id: str, *, cursor: Optional[str], limit: int) -> dict[str, Any]:
        raise NotImplementedError
...
class GmailClient(OAuthProviderClient):
    ...
    def authorization_url(self, *, redirect_uri: str, state: str, scopes: tuple[str, ...]) -> str:
        return GOOGLE_AUTH_URL + "?" + urlencode({"client_id": self.client_id, "redirect_uri": redirect_uri, "response_type": "code", "scope": " ".join(scopes), "access_type": "offline", "prompt": "consent", "include_granted_scopes": "true", "state": state})

    def complete_authorization(self, *, user_id: str, code: str, redirect_uri: str, scopes: tuple[str, ...]) -> dict[str, Any]:
        response = self.http.post(GOOGLE_TOKEN_URL, data={"client_id": self.client_id, "client_secret": self.client_secret, "code": code, "grant_type": "authorization_code", "redirect_uri": redirect_uri})
        ...
        record = OAuthTokenRecord(owner_user_id=user_id, provider=self.provider, account_id=address.lower(), address=address, access_token=token["access_token"], refresh_token=token.get("refresh_token"), expires_at=int(time.time()) + int(token.get("expires_in", 3600)), scopes=tuple(token.get("scope", " ".join(scopes)).split()), display_name=address)
    ...
    def refresh(self, record: OAuthTokenRecord) -> OAuthTokenRecord:
        ...
        response = self.http.post(GOOGLE_TOKEN_URL, data={"client_id": self.client_id, "client_secret": self.client_secret, "refresh_token": record.refresh_token, "grant_type": "refresh_token"})
        response.raise_for_status()
        token = response.json()
        return replace(record, access_token=token["access_token"], expires_at=int(time.time()) + int(token.get("expires_in", 3600)), scopes=tuple(token.get("scope", " ".join(record.scopes)).split()))
    ...
    def _walk_parts(self, payload: dict[str, Any]) -> tuple[str, str, list[dict[str, Any]]]:
        ...
        while stack:
            ...
            if filename and body.get("attachmentId"):
                attachments.append({"attachment_id": body["attachmentId"], "filename": filename, "mime_type": mime, "size_bytes": body.get("size")})
            elif body.get("data"):
    ...
    def fetch_messages(self, user_id: str, account_id: str, *, cursor: Optional[str], limit: int) -> dict[str, Any]:
        ...
        for ref in page.json().get("messages", []):
            item = self.http.get(f"{GMAIL_API}/users/me/messages/{ref['id']}", headers=headers, params={"format": "full"})
    ...
    def fetch_attachment(self, user_id: str, account_id: str, provider_message_id: str, attachment_id: str) -> dict[str, Any]:
        token = self._valid_access_token(user_id, account_id)
        response = self.http.get(f"{GMAIL_API}/users/me/messages/{provider_message_id}/attachments/{attachment_id}", headers={"Authorization": f"Bearer {token}"})
        response.raise_for_status()
...
class OutlookClient(OAuthProviderClient):
    ...
    def authorization_url(self, *, redirect_uri: str, state: str, scopes: tuple[str, ...]) -> str:
        return self.auth_url + "?" + urlencode({"client_id": self.client_id, "response_type": "code", "redirect_uri": redirect_uri, "response_mode": "query", "scope": " ".join(scopes), "state": state})

    def complete_authorization(self, *, user_id: str, code: str, redirect_uri: str, scopes: tuple[str, ...]) -> dict[str, Any]:
        response = self.http.post(self.token_url, data={"client_id": self.client_id, "client_secret": self.client_secret, "code": code, "redirect_uri": redirect_uri, "grant_type": "authorization_code", "scope": " ".join(scopes)})
        response.raise_for_status()
        token = response.json()
        headers = {"Authorization": f"Bearer {token['access_token']}"}
        ...
        record = OAuthTokenRecord(owner_user_id=user_id, provider=self.provider, account_id=str(me["id"]), address=str(address), access_token=token["access_token"], refresh_token=token.get("refresh_token"), expires_at=int(time.time()) + int(token.get("expires_in", 3600)), scopes=tuple(token.get("scope", " ".join(scopes)).split()), display_name=me.get("displayName"))
        self.store.upsert(record)
        return {"account_id": record.account_id, "address": record.address, "display_name": record.display_name}
    ...
    def refresh(self, record: OAuthTokenRecord) -> OAuthTokenRecord:
        ...
        response = self.http.post(self.token_url, data={"client_id": self.client_id, "client_secret": self.client_secret, "refresh_token": record.refresh_token, "grant_type": "refresh_token", "scope": " ".join(record.scopes)})
        response.raise_for_status()
        token = response.json()
        return replace(record, access_token=token["access_token"], refresh_token=token.get("refresh_token") or record.refresh_token, expires_at=int(time.time()) + int(token.get("expires_in", 3600)), scopes=tuple(token.get("scope", " ".join(record.scopes)).split()))
    ...
    def fetch_messages(self, user_id: str, account_id: str, *, cursor: Optional[str], limit: int) -> dict[str, Any]:
        ...
        for raw in page.json().get("value", []):
            ...
            if raw.get("hasAttachments"):
                att = self.http.get(f"{GRAPH_API}/me/messages/{raw['id']}/attachments", headers=headers, params={"$select": "id,name,contentType,size,isInline"})
                att.raise_for_status()
                attachments = [{"attachment_id": x["id"], "filename": x.get("name", "attachment"), "mime_type": x.get("contentType", "application/octet-stream"), "size_bytes": x.get("size")} for x in att.json().get("value", []) if not x.get("isInline")]
    ...
    def fetch_attachment(self, user_id: str, account_id: str, provider_message_id: str, attachment_id: str) -> dict[str, Any]:
        token = self._valid_access_token(user_id, account_id)
        response = self.http.get(f"{GRAPH_API}/me/messages/{provider_message_id}/attachments/{attachment_id}", headers={"Authorization": f"Bearer {token}"})
        response.raise_for_status()
        raw = response.json()
